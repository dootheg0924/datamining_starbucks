...
def require_or_build_district_scores() -> None:
    scores_path = DISTRICT_CLASSIFICATION_OUTPUT_DIR / "district_scores.csv"
    university_path = DISTRICT_CLASSIFICATION_OUTPUT_DIR / "district_university.csv"
...
def read_inputs() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    personas = pd.read_parquet(CAFE_CLASSIFICATION_OUTPUT_DIR / "cafe_personas.parquet").reset_index(drop=True)
    scores = pd.read_csv(DISTRICT_CLASSIFICATION_OUTPUT_DIR / "district_scores.csv", encoding="utf-8-sig")
...
def load_university_scores() -> pd.DataFrame:
    scores = pd.read_csv(DISTRICT_CLASSIFICATION_OUTPUT_DIR / "district_university.csv", encoding="utf-8-sig")
...
def write_tables(bridge: pd.DataFrame, universities: pd.DataFrame, decile: pd.DataFrame, top_districts: pd.DataFrame) -> None:
    ...
    for filename, frame in targets.items():
        frame.to_csv(DISTRICT_CLASSIFICATION_OUTPUT_DIR / filename, index=False, encoding="utf-8-sig")
        frame.to_csv(ARCHIVE_TABLE_DIR / filename, index=False, encoding="utf-8-sig")
    bridge.to_parquet(DISTRICT_CLASSIFICATION_OUTPUT_DIR / "district_persona_bridge.parquet", index=False)


def savefig(fig: plt.Figure, filename: str) -> None:
    generated = DISTRICT_CLASSIFICATION_FIGURE_DIR / filename
...
def validate_outputs(bridge: pd.DataFrame, universities: pd.DataFrame) -> None:
    ...
    required = [
        DISTRICT_CLASSIFICATION_OUTPUT_DIR / "district_persona_bridge.csv",
        DISTRICT_CLASSIFICATION_OUTPUT_DIR / "district_university_persona.csv",
        DISTRICT_CLASSIFICATION_OUTPUT_DIR / "district_persona_decile_summary.csv",
        DISTRICT_CLASSIFICATION_OUTPUT_DIR / "top20_district_persona.csv",
        DISTRICT_CLASSIFICATION_OUTPUT_DIR / "district_persona_bridge.parquet",
...
def main() -> None:
    ...
    print(f"[bridge] tables: {DISTRICT_CLASSIFICATION_OUTPUT_DIR}")
